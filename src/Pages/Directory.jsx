...
function Directory() {
  ...
  const [filteredData, setFilteredData] = useState(siteData.filters);
  ...
  const handleSearch = (query, filter) => {
    ...
    setFilteredData((prevData) => ({
      ...prevData,
      [filter]: siteData.filters[filter].filter((item) =>
  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col md:flex-row px-4">
        <button
          className="bg-black text-white w-max p-2 rounded-r-lg fixed top-12 left-0 inline md:hidden"
        ...
        <div
          ref={filtersRef}
          className={`absolute md:relative scroll-smooth h-screen top-0 pt-8 p-2 left-0 md:top-auto md:left-0 md:pt-0 z-10 md:z-auto md:w-auto min-w-[310px] md:h-[80vh] mt-8 flex flex-col md:pr-8 gap-4 bg-white md:border-r-2 border-[rgba(0,0,0,0.1)] duration-300 ${
            active ? "translate-x-0" : "md:translate-x-0 -translate-x-[80vw]"
          }`}
        >
          {/* Filters */}
          {Object.keys(filteredData).map((filter, key) => (
            <div className="w-full flex flex-col gap-2" key={key}>
              <div className="flex items-center gap-2">
                <h1
                  className="font-bold bg-[#9f70fd] text-white rounded px-4 py-2 flex items-center transition relative"
                  id={`head${key + 1}`}
                >
                  {filter}
                </h1>
              ...
              <div className="pl-4 max-w-[64vw] flex-wrap mt-2">
                {filteredData[filter].slice(0, 5).map((item, index) => (
                  <label
                    key={index}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selectedValues.includes(item.toString())}
        ...
        <div className="flex flex-col justify-center mt-8">
          <p className="text-4xl text-center md:text-start md:ml-8 text-black font-bold py-8 border-b-2 border-[rgba(0,0,0,0.1)]">
            Alumni Directory
          </p>
          {/* Cards */}
          <div
            className="h-[66vh] overflow-hidden md:ml-8 mt-12"
            data-lenis-prevent
          >
            <div className="flex flex-wrap gap-10 justify-center h-[50vh] overflow-y-scroll">
              {siteData.alumniData.map((items, index) => (
                <div
                  key={index}
                  className="flex flex-col h-[50vh] bg-white rounded-lg overflow-hidden relative"
                >
                  <img
                  ...
                  />
                  <div className="px-4 absolute bottom-0 left-0 text-white bg-[rgba(0,0,0,0.4)] backdrop-blur-[2px] w-full py-2">
                    <p className="text-lg mb-2 border-b-[1px] border-[rgba(255,255,255,.5)]">
                      {items.name}
                    </p>
